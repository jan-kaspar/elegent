...
TComplex CmplxDeriv(TComplex (*fcn)(double*, double*), double x, double *params, double h)
{
  double xx[1];
  
  // if x is 0 (or too small), computes one side derivation
  if (x < h)
  {
    printf("too small, %f\n", x);
    xx[0] = x;	TComplex fx = (*fcn)(xx, params);
    xx[0] = x + h;	TComplex fxh = (*fcn)(xx, params);
    return (fxh - fx) / h;
  }
  
  xx[0] = x+h;     TComplex f1 = (*fcn)(xx, params);
  xx[0] = x;       TComplex fx = (*fcn)(xx, params);
  xx[0] = x-h;     TComplex f2 = (*fcn)(xx, params);
  
  xx[0] = x+h/2;   TComplex g1 = (*fcn)(xx, params);
  xx[0] = x-h/2;   TComplex g2 = (*fcn)(xx, params);
